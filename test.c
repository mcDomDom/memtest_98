...
void sleep(int n, int sms)
{
	...
	if (0/*v->rdtsc*/) {
		...
		for (i=0; i<n*1000; i++)	outb(1, 0x005F);
	}
}
...
{
	char systype = inb_p(0x42);
	unsigned int sysclock = (systype & 0x04) ? 1996800 : 2457600 ;	// 8MHz or 5,10MHz
	unsigned int count = sysclock / frequency;
