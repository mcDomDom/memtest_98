...
static void __run_at(unsigned long addr)
{
	...
	cprint(2, 0, "_end");
	hprint(2, 16, _end);
	cprint(3, 0, "startup_32");
	hprint(3, 16, startup_32);
	cprint(4, 0, "high_test_adr");
	hprint(4, 16, high_test_adr);
	cprint(5, 0, "LOW_TEST_ADR");
	hprint(5, 16, LOW_TEST_ADR);
	cprint(6, 0, "high_test_adr&");
	hprint(6, 16, &high_test_adr);
	cprint(7, 0, "e88_mem_k");
	hprint(7, 16, mem_info.e88_mem_k);
	cprint(8, 0, "e801_mem_k");
	hprint(8, 16, mem_info.e801_mem_k);
	cprint(9, 0, "e820_nr;");
	hprint(9, 16, mem_info.e820_nr);
	for (i=0; i<16; i++) {
		hprint(10+i, 0, mem_info.e820[i].addr);
		hprint(10+i, 10, mem_info.e820[i].size);
		hprint(10+i, 20, mem_info.e820[i].type);
	}
	*/
